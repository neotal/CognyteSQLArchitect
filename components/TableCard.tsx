...
import { Key, Table as TableIcon, Hash, Link, Layers, MoreVertical, Edit2, Trash2, Info, ChevronRight } from 'lucide-react';
...
const TableCard: React.FC<TableCardProps> = memo(({ 
  ...
  return (
    <div 
      className="absolute bg-white rounded-2xl shadow-xl border-2 select-none group/table overflow-visible transition-all hover:shadow-2xl hover:-translate-y-0.5"
      style={{ 
    ...
    >
      ...
      <div 
        className="relative px-4 py-3 flex items-center justify-between border-b cursor-move rounded-t-xl overflow-visible"
          ...
          {isHoveringName && table.description && (
            <div className="absolute left-0 bottom-full mb-3 w-64 p-4 bg-slate-900 text-white text-xs rounded-2xl shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
              <div className="font-black mb-1.5 flex items-center text-blue-400 uppercase tracking-widest text-[9px]">
                <Info className="w-3 h-3 mr-1.5" /> Table Info
              </div>
      ...
      <div className="relative max-h-60 overflow-y-auto bg-white/40 backdrop-blur-[1px] custom-scrollbar">
        {keys.map(col => (
      ...
      <div className="relative px-3 py-2 bg-slate-50/95 flex items-center justify-around border-t border-slate-100 rounded-b-xl">
        <button 
          onMouseDown={(e) => e.stopPropagation()}
        ...
        <div className="absolute top-full left-0 right-0 mt-3 bg-white shadow-2xl rounded-2xl border border-slate-200 p-4 z-40 animate-in slide-in-from-top-2 duration-300">
          <div className="flex justify-between items-center mb-3">
            <span className="text-[9px] font-black uppercase text-slate-400 tracking-widest flex items-center">
              {showStats === 'cols' ? 'Columns List' : showStats === 'rels' ? 'Connected To' : 'Member Of'}
